...
MAIN	BSF 	STATUS,RP0 ; BANK를 1로 변경함
	MOVLW	B'00010111'; 
	MOVWF	TRISA
	MOVLW	B'00000000';
...
LP 	MOVF	DISP_B,W ; 표시할 값 W로
	CALL	DISP	 ; W값을 표시함
	BTFSS	PORTA,0	 ; 암호 입력 스위치 눌러진지 확인
	GOTO	PS0
...
PS12	MOVLW	.9	; UDP 설정 
	...
	CALL	DISP
	CALL	DELAY
	BTFSC	PORTA,4 ; 암호 입력 스위치 눌러짐 확인
...
;저장위치 구분
	...
	GOTO 	S_4
S_1	MOVF	DISP_B,W
	MOVWF	UDP_1 	; 1번째로 들어온 암호 저장
	GOTO	LP6
S_2	MOVF	DISP_B,W
	MOVWF	UDP_2 	; 2번째로 들어온 암호 저장
	GOTO	LP6
S_3	MOVF	DISP_B,W
	MOVWF	UDP_3 	; 3번째로 들어온 암호 저장
	GOTO	LP6
S_4	MOVF	DISP_B,W
	MOVWF	UDP_4 	; 4번째로 들어온 암호 저장
	BSF	PORTA,5 ; 부져 ON
	CALL	DELAY_1
	BCF	PORTA,5 ; 부져 OFF
	BSF	PORTA,5 ; 부져 ON
...
;암호가 다 들어왓으니 암호 검사
	MOVF	UDP_1,W	; 1번째 내부 암호 값
	SUBWF	PASS_1,W
	BTFSS 	STATUS, ZF
	GOTO	LP5 	; 아닌경우 분기
	MOVF	UDP_2,W	; 2번째 내부 암호 값
	SUBWF	PASS_2,W
	BTFSS 	STATUS, ZF
	GOTO	LP5 	; 아닌경우 분기
	MOVF	UDP_3,W	; 3번째 내부 암호 값
	SUBWF	PASS_3,W
	BTFSS 	STATUS, ZF
	GOTO	LP5 	; 아닌경우 분기
	MOVF	UDP_4,W	; 4번째 내부 암호 값
	SUBWF	PASS_4,W
	BTFSS 	STATUS, ZF
	GOTO	LP5 	; 아닌경우 분기
;모든 암호가 맞았으므로 자물쇠 ON
